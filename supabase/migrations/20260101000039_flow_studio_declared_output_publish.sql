-- Flow Studio：只允许显式输出节点发布到 Canvas，禁止发布中间执行结果。
  ...
  for v_output in
    select o.*, a.kind, a.width as asset_width, a.height as asset_height
     ...
     where n.run_id = p_run_id and o.id = any(p_output_ids)
       and n.kind in ('text_output', 'image_output', 'video_output', 'gallery_output')
       and (
         (o.value_type = 'text' and o.asset_id is null)
         or (o.value_type in ('image_asset', 'video_asset', 'image_list') and a.id is not null)
       )
